...
public class OneAwayDetector {
    ...
    public boolean oneAway(String first, String second) {
        ...
        if (Math.abs(first.length() - second.length()) > 1) { //MUTLAK DEĞER
            return false;        
    ...
    private boolean OneEditInsert(String s1, String s2) {
        ...
        while (j < s2.length() && i < s1.length()) {
            if (s1.charAt(i) != s2.charAt(j)) {
                if (i != j) { //burada tek hakkın var anlamı var. kısacası 1 harf fazla geldiginde i ve j birseferlik eşit olmuyor,sonra bir adet daha gelirse harf burada false dönüyor.
                    return false;
                }
                j++;
            } else {
    ...
    private boolean OneEditReplace(String s1, String s2) {
        ...
        for (int i = 0; i < s1.length(); i++) {
            if (s1.charAt(i) != s2.charAt(i)) {
                if (foundDifference) { //burada tek hakkın var degisik olması icin demek. asagida true yaptıktan sonra birdaha girerse false döndürüp bitiriyor.
                    return false;
                }
